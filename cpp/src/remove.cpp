...
int FileSystem::remove(const std::string_view filename) {
  if (!state_ || !state_->mounted() || filename.empty()) {
    return 1;
  }
  const int len_fn = static_cast<int>(filename.size());
  if (filename.back() == '/') {
    return 5;
  }
  ...
  std::uint32_t file_start = 0U;
  std::uint32_t file_stop = 0U;
  ...
  for (;;) {
    ...
    for (int i = 0; i < detail::BlockItemMaxCount; ++i) {
      ...
      file_start = detail::b4_to_u32(std::span<const std::uint8_t, 4>(block.data() + k, 4));
      file_stop = detail::b4_to_u32(std::span<const std::uint8_t, 4>(block.data() + k + 4, 4));
  ...
  if (file_start > 0U) {
    detail::Block file_block_stop{};
    if (!state_->readblock(file_stop, file_block_stop)) {
      if (state_->tmp.state == 1U) {
        state_->tmpstop();
      }
      return 1;
    }
    detail::u32_to_b4(state_->tmp.new_unused_blockhead, std::span<std::uint8_t, 4>(file_block_stop.data() + 4, 4));
    state_->tmp.new_unused_blockhead = file_start;
    if (!state_->writeblock(file_stop, file_block_stop)) {
      if (state_->tmp.state == 1U) {
        state_->tmpstop();
      }
      return 1;
